...
int main()
{
	...
	*pwm_enable = 0;
	...
	commandType type = TYPE_LIGHT;
	struct commandLight cl;
	cl.state = LIGHTS_ON;
	cl.type = LIGHTS_HEAD;

	CommandManager com_mgr (sender);
	com_mgr.sendCommand(type, cl);

	delay(1000);

	Command* command_ptr = com_mgr.readLastCommand();
	commandType type_new = command_ptr->type;
	commandSender sender_new = command_ptr->sender;

	void* command_new = command_ptr->command;
	switch(type_new) {
	case TYPE_LIGHTS:
		struct commandLight cl_new = (commandLight) command_new;
		printf("%d\n", cl_new.state);
		break;
	case TYPE_WHEELS:
		struct commandWheel cw_new = (commandWheel) command_new;
		break;
	case TYPE_EMERGENCY:
		struct commandEmergency ce_new = (commandEmergency) command_new;
		break;
	case TYPE_PARKING:
		struct commandParking cp_new = (commandParking) command_new;
		break;
	default: break;
	}
